...
$ sample/hsqldb/bin/sync-sample --import sample/hsqldb/sample.csv
$ sample/hsqldb/bin/sync-sample --run

Then, launch the SYNC in a command line to start LDAP to postgresql synchronization:
$ bin/lsc -f sample/postgresql/etc-opendj2postgresql -s all -c all

Or launch the SYNC in a command line to start an asynchronous postgresql to LDAP task:
